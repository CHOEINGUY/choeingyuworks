...
function doGet() {
  var template = HtmlService.createTemplateFromFile('index3.0');
  return template.evaluate()
    .setTitle('남원코호트 진행 현황 - 3.0 Version')
...
function getFilteredTableData() {
  ...
  // SNSB-C, SNSB-C시작, SNSB-C종료, 수행자, 채혈, 채혈시작, 채뇨, 신체계측, 신체계측시작, 신체계측종료
  ...
  var outputIndexes = [1, 0, 5, 7, 8, 11, 13, 14, 15, 16, 17, 18, 19, 20];
  var outputHeader = [
    "이름", "ID", "심전도", "심전도 시작", "심전도 종료",
    "신체기능", "신체기능 시작", "신체기능 종료", "SNSB-C", "SNSB-C 시작", 
    "SNSB-C 종료", "수행자", "채혈", "채혈 시작", "채뇨", "신체계측", "신체계측 시작", "신체계측 종료"
  ];
  ...
  var outputRows = rows.map(function(row) {
    return outputIndexes.map(function(idx, i) {
      if (i === 1) {  // ID 열
        ...
        var measurementStatus = row[18];  // 신체계측 상태
        
        var statusClass = 'red';  // 기본값: 검사중이지 않음
        
        // 모든 검사가 완료된 경우
        if (ecgStatus === '완료' && physicalStatus === '완료' && 
            snsbcStatus === '완료' && bloodStatus === '완료' && measurementStatus === '완료') {
          statusClass = 'black';
        }
        // 하나라도 검사중인 경우
        else if (ecgStatus === '검사중' || physicalStatus === '검사중' || 
                 snsbcStatus === '검사중' || bloodStatus === '검사중' || measurementStatus === '검사중') {
          statusClass = 'green';
        }
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (overallStatus === '출석') {
        ...
        var measurementStatus = row[18]; // '신체계측'
        ...
        if (!hasAnyTestInProgress) {
          ...
          // 신체계측 다음 대상자
          if (!foundMeasurement && measurementStatus !== '완료' && measurementStatus !== '검사중' && measurementStatus !== '진행중') {
    ...
    if (foundEcg && foundPhysical && foundSnsbc && foundBlood && foundMeasurement) break;
  }
