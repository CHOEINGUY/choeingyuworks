...
function doGet() {
  ...
  function getFilteredTableData2() {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      // 어떤 검사든 "검사중"이면 다음 대상자에서 제외
      var isOtherTestInProgress = (ecgStatus === '검사중' || physicalStatus === '검사중' || snsbcStatus === '검사중');
  ...
  function getFilteredTableData() {
    ...
    var outputIndexes = [1, 0, 5, 8, 11, 6, 9, 12, 14, 15, 16, 17, 18, 19, 20];
    var outputHeader = [
    ...
    var outputRows = rows.map(function(row) {
      return outputIndexes.map(function(idx, i) {
        if (i === 1) {  // ID 열 (두 번째 열)
          ...
          // 모든 검사가 완료된 경우
          if (ecgStatus === '완료' && physicalStatus === '완료' && snsbcStatus === '완료') {
            statusClass = 'black';
          }
          // 하나라도 검사중인 경우
          else if (ecgStatus === '검사중' || physicalStatus === '검사중' || snsbcStatus === '검사중') {
            statusClass = 'green';
          }
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (overallStatus === '출석') {
          ...
          // 어떤 검사든 하나라도 검사중인 사람은 모든 다음 대상자에서 제외
          var hasAnyTestInProgress = (ecgStatus === '검사중' || physicalStatus === '검사중' || snsbcStatus === '검사중');
