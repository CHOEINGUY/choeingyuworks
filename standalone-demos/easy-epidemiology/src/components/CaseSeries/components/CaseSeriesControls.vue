  ...
  <div class="w-full flex justify-between items-center px-5 py-4 border-b border-slate-100">
    <div class="flex gap-5">
      <div class="flex items-center gap-2.5">
        <label class="font-semibold text-slate-700 text-sm">{{ $t('caseSeries.controls.fontSize') }}:</label>
    ...
    <div class="text-sm font-medium text-slate-500">
      <p class="m-0">{{ $t('caseSeries.controls.totalCount', { count: rowCount }) }}</p>
    </div>
    </div>
  </div>
</template>
...
const fontSizeButtonText = computed(() => {
  ...
  if (currentIndex === 0) return t('caseSeries.controls.small');
  if (currentIndex === 1) return t('caseSeries.controls.medium');
  if (currentIndex === 2) return t('caseSeries.controls.large');
  return t('caseSeries.controls.medium');
});
...
const handleFontSizeMouseEnter = () => {
  ...
  showTooltip('fontSize', t('caseSeries.controls.fontSizeTooltip', { size: nextFontSize }));
};
