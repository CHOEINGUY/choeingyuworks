...
export const AdminScene = ({ onComplete }: { onComplete: () => void }) => {
    ...
    useEffect(() => {
        ...
        const scenario = async () => {
            await wait(500); if (!active) return;
            setNotification("🔔 새로운 참가 신청이 도착했습니다!");
            await wait(1200); if (!active) return;
            setNotification(null);
            setShowNewRow(true);
            await wait(600); if (!active) return; await click(); setSelectedApplicant(NEW_APPLICANT);
            await wait(1200); if (!active) return; await click(); setIsApproved(true);
            await wait(1000); if (!active) return; await click();
            ...
            await wait(1500); if (!active) return; onComplete();
        };
        scenario();
        return () => { active = false; };
    }, []);
