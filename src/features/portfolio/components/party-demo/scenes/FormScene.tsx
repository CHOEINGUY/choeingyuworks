...
export const FormScene = ({ onComplete }: { onComplete: () => void }) => {
    ...
    const typeName = async (text: string) => { for (let i = 1; i <= text.length; i++) { if (!mounted.current) return; setName(text.slice(0, i)); await wait(60); } };
    const typePhone = async (text: string) => { for (let i = 1; i <= text.length; i++) { if (!mounted.current) return; setPhone(text.slice(0, i)); await wait(40); } };

    useEffect(() => {
        let active = true;
        const sequence = async () => {
            if (step === 'cover') { await wait(600); if (active) { await click(); await wait(150); if (active) setStep('name'); } }
            if (step === 'name') { await wait(400); if (active) { await click(); await typeName("최인규"); await wait(250); if (active) await click(); if (active) setStep('phone'); } }
            if (step === 'phone') { await wait(350); if (active) { await click(); await typePhone("010-1234-5678"); await wait(250); if (active) await click(); if (active) setStep('schedule'); } }
            if (step === 'schedule') { await wait(500); if (active) { await click(); setSelectedSchedule('fri'); await wait(350); if (active) await click(); if (active) setStep('photo'); } }
            if (step === 'photo') { await wait(500); if (active) { await click(); for (let i = 0; i <= 100; i += 20) { if (!active) break; setPhotoProgress(i); await wait(40); } await wait(250); if (active) await click(); if (active) setStep('payment'); } }
            if (step === 'payment') { await wait(800); if (active) { await click(); await wait(150); if (active) setStep('success'); } }
            if (step === 'success') { await wait(800); if (active) onComplete(); }
        };
        sequence();
        return () => { active = false; };
    }, [step]);
                ...
                {step === 'photo' && (
                    <motion.div key="photo" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex-1 flex flex-col p-6 justify-center pb-32 overflow-y-auto">
                        ...
                        <div className="aspect-[3/4] w-full max-w-[240px] mx-auto bg-gray-50 border-2 border-dashed border-gray-300 rounded-2xl flex flex-col items-center justify-center gap-3 relative overflow-hidden" style={{ scale: isClicking && photoProgress === 0 ? 0.98 : 1 }}>
                            {photoProgress === 0 ? (<><div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm text-gray-400"><Camera size={32} /></div><span className="font-bold text-gray-500 text-sm">사진 업로드하기</span></>) : (<><img src="https://images.unsplash.com/photo-1595152452543-e5fc28ebc2b8?fit=crop&w=400&h=600" className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-500 ${photoProgress === 100 ? 'opacity-100' : 'opacity-50 blur-sm'}`} />{photoProgress < 100 && (<div className="absolute inset-x-8 top-1/2 rounded-full h-2 bg-gray-200 overflow-hidden z-10"><div className="h-full bg-blue-500 transition-all duration-100" style={{ width: `${photoProgress}%` }} /></div>)}</>)}
                        </div>
                    </motion.div>
                )}
                {step === 'payment' && (
